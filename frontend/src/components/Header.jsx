...
export default function Header({ open, setOpen }) {
    ...
    return (
        <>
            ...
            {/* Sidebar */}
            <div
                className={`sidebar bg-a text-p p-3 border-end border-2 border-black ${!open ? "d-none" : ""} d-md-block`}
                style={{
                    width: open ? '250px' : '70px',
                }}
            >
                {/* Logo */}
                <div className={`mb-5 border-bottom border-2 border-black pb-2 mt-5 ${!open ? "text-center" : ""}`}>
                    <NavLink to="/" className="text-decoration-none text-p fw-bold fs-4">
                        {open ? "CreditFlow" : "CF"}
                    </NavLink>
                </div>

                {/* Menu */}
                <ul className="list-unstyled d-flex flex-column gap-3 fs-4 fw-bold">

                    <li className={`mb-3 border border-black rounded-3 p-1 ${open ? '' : 'd-flex justify-content-center'}`}>
